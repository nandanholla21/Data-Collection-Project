#all english league teams and player squad extraction
...
driver.get("https://www.espn.co.uk/football/table/_/league/eng.2")
english_league_teams=[]             #get the names of all english league teams
english_league_teams_link=[]        #get the link of all english league teams
english_league_team_image=[]
tag_name="abbr"
li = driver.find_elements(by=By.TAG_NAME,value=tag_name)
for row in li:
    english_league_teams.append(row.get_attribute("title"))
...
for row in img_tag:                                       #
    english_league_team_image.append(row.get_attribute("src")) #
for row in atag:
    if row.get_attribute("href") not in english_league_teams_link:
        english_league_teams_link.append(row.get_attribute("href"))
#print(english_league_teams_link)
for i in range(len(english_league_teams_link)):
    team = english_league_teams[i]
    link = english_league_teams_link[i]
    team_img = english_league_team_image[i]
    driver.get(link)
...
#print(len(english_league))
# print(english_league_team_image)
conn = sqlite3.connect("E:\Project X\Data.db")
cursor = conn.cursor()
cursor.execute('''
create table if not exists english_league(
Team TEXT,
Name TEXT,
Team_logo TEXT
)''')
conn.commit()
for item in english_league:
    team = str(item['Team'])
    name = str(item['Name'])
    team_logo = str(item['team_logo'])
    cursor.execute('''insert into english_league values(?,?,?)''',(team,name,team_logo))
    conn.commit()
conn.close()