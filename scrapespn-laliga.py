...
driver.get("https://www.espn.co.uk/football/table/_/league/esp.1")
spanish_league_teams=[]             #get the names of all english league teams
spanish_league_teams_link=[]        #get the link of all english league teams
spanish_league_team_image=[]
tag_name="abbr"
li = driver.find_elements(by=By.TAG_NAME,value=tag_name)
for row in li:
    spanish_league_teams.append(row.get_attribute("title"))
...
for row in img_tag:                                       #
    spanish_league_team_image.append(row.get_attribute("src")) #
for row in atag:
    if row.get_attribute("href") not in spanish_league_teams_link:
        spanish_league_teams_link.append(row.get_attribute("href"))
#print(spanish_league_teams_link)
for i in range(len(spanish_league_teams_link)):
    team = spanish_league_teams[i]
    link = spanish_league_teams_link[i]
    team_img = spanish_league_team_image[i]
    driver.get(link)
...
# print(len(spanish_league))
# print(spanish_league_team_image)
conn = sqlite3.connect("E:\Project X\Data.db")
cursor = conn.cursor()
cursor.execute('''
create table if not exists spanish_league(
Team TEXT,
Name TEXT,
Team_logo TEXT
)''')
conn.commit()
for item in spanish_league:
    team = str(item['Team'])
    name = str(item['Name'])
    team_logo = str(item['team_logo'])
    cursor.execute('''insert into spanish_league values(?,?,?)''',(team,name,team_logo))
    conn.commit()
conn.close()