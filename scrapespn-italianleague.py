...
for row in li:
    italian_league_teams.append(row.get_attribute("title"))
...
cursor = conn.cursor()
cursor.execute('''
create table if not exists italian_league(
Team TEXT,
Name TEXT,
Team_logo TEXT
)''')
conn.commit()
for item in italian_league:
    team = str(item['Team'])
    name = str(item['Name'])
    team_logo = str(item['team_logo'])
    cursor.execute('''insert into italian_league values(?,?,?)''',(team,name,team_logo))
    conn.commit()
conn.close()