...
driver.get("https://www.espn.co.uk/football/table/_/league/fra.1")
french_league_teams=[]             #get the names of all english league teams
french_league_teams_link=[]        #get the link of all english league teams
french_league_team_image=[]
tag_name="abbr"
li = driver.find_elements(by=By.TAG_NAME,value=tag_name)
for row in li:
    french_league_teams.append(row.get_attribute("title"))
...
for row in img_tag:                                       #
    french_league_team_image.append(row.get_attribute("src")) #
for row in atag:
    if row.get_attribute("href") not in french_league_teams_link:
        french_league_teams_link.append(row.get_attribute("href"))
#print(spanish_league_teams_link)
for i in range(len(french_league_teams_link)):
    team = french_league_teams[i]
    link = french_league_teams_link[i]
    team_img = french_league_team_image[i]
    driver.get(link)
...
# print(len(french_league))
# print(french_league_team_image)
conn = sqlite3.connect("E:\Project X\Data.db")
cursor = conn.cursor()
cursor.execute('''
create table if not exists french_league(
Team TEXT,
Name TEXT,
Team_logo TEXT
)''')
conn.commit()
for item in french_league:
    team = str(item['Team'])
    name = str(item['Name'])
    team_logo = str(item['team_logo'])
    cursor.execute('''insert into french_league values(?,?,?)''',(team,name,team_logo))
    conn.commit()
conn.close()